...
function spPortalPage(c: any) { return `<!doctype html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>SP Portal</title><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet">${getCSS()}<script>window.api=async function(p,b){var r=await fetch(p,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(b||{})});return r.json();};window.loadList=async function(p){return api('/api/list',{prefix:p});};window.saveItem=async function(p,d){return api('/api/save',{prefix:p,data:d});};window.fmt=function(n){return Number(n||0).toLocaleString('en-IN');};window.todayISO=function(){return new Date().toISOString().slice(0,10);};window.txnNo=function(p){return p+'-'+todayISO().replace(/-/g,'')+'-'+Math.random().toString(36).slice(2,7).toUpperCase();};</script></head><body style="background:var(--bg);padding:20px"><div style="max-width:800px;margin:0 auto"><h2>Salesperson Portal</h2><p class="text-muted">Place orders for customers. <a href="/sp-portal/logout">Logout</a></p><div class="card" style="margin-top:16px"><h3 style="font-size:14px">Place Order</h3><div class="form-row"><div><label>Customer</label><select id="spCust"></select></div><div><label>Date</label><input type="date" id="spDate"></div></div><div id="spItems"></div><button class="btn btn-outline btn-sm" onclick="spI.push({pk:'',pn:'',q:1,r:0,a:0});renderSI()">+ Item</button><button class="btn btn-primary" style="margin-left:8px" onclick="submitOrd()">Submit</button></div><div class="card" style="margin-top:14px"><h3 style="font-size:14px">My Orders</h3><div id="spOrders"></div></div></div><script>var spP=[],spC=[],spI=[{pk:'',pn:'',q:1,r:0,a:0}];(async function(){var d=await Promise.all([loadList('product:'),loadList('party:')]);spP=d[0]||[];spC=(d[1]||[]).filter(function(p){return p.type==='customer';});document.getElementById('spCust').innerHTML='<option value="">Select</option>'+spC.map(function(c){return'<option value="'+c._key+'">'+c.name+'</option>';}).join('');document.getElementById('spDate').value=todayISO();renderSI();})();window.renderSI=function(){document.getElementById('spItems').innerHTML=spI.map(function(it,i){return'<div class="form-row" style="grid-template-columns:2fr 60px 80px 80px 24px;align-items:end;margin-bottom:4px"><select onchange="var p=spP.find(function(x){return x._key===this.value;}.bind(this));if(p){spI['+i+'].pk=p._key;spI['+i+'].pn=p.name;spI['+i+'].r=p.salePrice||0;spI['+i+'].a=spI['+i+'].q*spI['+i+'].r;renderSI();}"><option>Product</option>'+spP.filter(function(p){return p.stock>0;}).map(function(p){return'<option value="'+p._key+'" '+(it.pk===p._key?'selected':'')+'>'+p.name+'</option>';}).join('')+'</select><input type="number" value="'+it.q+'" oninput="spI['+i+'].q=+this.value;spI['+i+'].a=spI['+i+'].q*spI['+i+'].r;renderSI()"><input value="'+fmt(it.r)+'" readonly><div style="font-weight:700;font-size:11px">'+fmt(it.a)+'</div><button class="btn btn-danger btn-xs" onclick="spI.splice('+i+',1);if(!spI.length)spI.push({pk:\\'\\',pn:\\'\\',q:1,r:0,a:0});renderSI()">x</button></div>';}).join('');};window.submitOrd=async function(){var ck=document.getElementById('spCust').value;var c=spC.find(function(x){return x._key===ck;});if(!c)return alert('Select customer');var v=spI.filter(function(i){return i.pk&&i.q>0;});if(!v.length)return alert('Add items');var t=v.reduce(function(s,i){return s+i.a;},0);await saveItem('order:',{date:document.getElementById('spDate').value,orderNo:txnNo('ORD'),customerId:c._key,customerName:c.name,salespersonId:'sp',salespersonName:'SP',items:v.map(function(i){return{productKey:i.pk,productName:i.pn,qty:i.q,rate:i.r,amount:i.a};}),total:t,status:'pending'});alert('Order submitted!');location.reload();};</script></body></html>`; }

function dashboardPage(){var t=new Date().toISOString().slice(0,10);return `
...
document.getElementById('stats').innerHTML=[{l:'Total Sales',v:fmt(totalSales),c:'var(--accent)'},{l:'Total Purchases',v:fmt(totalPurchases),c:'var(--primary)'},{l:'Expenses',v:fmt(totalExpenses),c:'var(--warning)'},{l:'Receivables',v:fmt(receivables),c:'var(--info)'},{l:'Payables',v:fmt(payables),c:'var(--danger)'},{l:'Cash Flow',v:fmt(cashFlow),c:cashFlow>=0?'var(--accent)':'var(--danger)'},{l:'Products',v:products.length,c:'var(--primary)'},{l:'Pending Orders',v:orders.filter(function(o){return o.status==='pending'}).length,c:'var(--warning)'}].map(function(s){return'<div class="stat"><div class="label">'+s.l+'</div><div class="value" style="color:'+s.c+'">'+s.v+'</div></div>'}).join('');
...
function renderP(l){document.getElementById('pBody').innerHTML=!l.length?'<tr><td colspan="6" class="empty">No products</td></tr>':l.map(function(p){return'<tr><td class="bold">'+p.name+'</td><td><span class="badge badge-info">'+(p.sku||'')+'</span></td><td class="r">'+fmt(p.purchasePrice)+'</td><td class="r">'+fmt(p.salePrice)+'</td><td class="r bold '+((p.stock||0)<=0?'text-danger':(p.stock||0)<=5?'text-warning':'')+'">'+fmt(p.stock)+'</td><td class="r"><button class="btn btn-outline btn-sm" onclick="editP(\''+p._key+'\')">E</button> <button class="btn btn-danger btn-sm" onclick="delP(\''+p._key+'\')">D</button></td></tr>'}).join('')}
...
<div class="card" style="padding:0"><div class="table-wrap"><table class="tbl"><thead><tr><th>Name</th><th>Phone</th><th>Address</th><th class="r">Credit Limit</th><th class="r">Balance</th><th class="r">Act</th></tr></thead><tbody id="paBody"></tbody></table></div></div>
<div class="modal-overlay" id="addParty"><div class="modal"><h3 id="paTitle">Add</h3><input type="hidden" id="paEK"><div class="form-group"><label>Name</label><input id="paN"></div><div class="form-row"><div><label>Phone</label><input id="paPh"></div><div><label>Credit Limit</label><input type="number" id="paCL" placeholder="0=none"></div></div><div class="form-group"><label>Address</label><input id="paAd"></div><div style="display:flex;gap:6px;justify-content:flex-end;margin-top:12px"><button class="btn btn-outline" onclick="closeModal('addParty')">Cancel</button><button class="btn btn-primary" onclick="savePa()">Save</button></div></div></div>
<script>
var aP=[],pT='customer';
window.switchPT=function(t,el){pT=t;document.querySelectorAll('.tab').forEach(function(x){x.classList.remove('active')});el.classList.add('active');renderPa()}
window.openAddParty=function(){document.getElementById('paTitle').textContent='Add '+(pT==='customer'?'Customer':'Supplier');document.getElementById('paEK').value='';['paN','paPh','paAd'].forEach(function(i){document.getElementById(i).value=''});document.getElementById('paCL').value='';openModal('addParty')}
window.savePa=async function(){var ek=document.getElementById('paEK').value;var n=document.getElementById('paN').value.trim();if(!n)return alert('Name required');var d={name:n,phone:document.getElementById('paPh').value.trim(),address:document.getElementById('paAd').value.trim(),creditLimit:+document.getElementById('paCL').value||0};if(ek){var old=aP.find(function(x){return x._key===ek});if(old)d=Object.assign({},cleanForSave(old),d);await saveByKey(ek,d)}else{d.type=pT;d.balance=0;await saveItem('party:',d)}closeModal('addParty');loadPa()}
window.editPa=function(k){var p=aP.find(function(x){return x._key===k});if(!p)return;pT=p.type;document.getElementById('paTitle').textContent='Edit';document.getElementById('paEK').value=p._key;document.getElementById('paN').value=p.name||'';document.getElementById('paPh').value=p.phone||'';document.getElementById('paAd').value=p.address||'';document.getElementById('paCL').value=p.creditLimit||'';openModal('addParty')}
async function loadPa(){aP=await loadList('party:');renderPa()}
function renderPa(li){var l=li||aP.filter(function(p){return p.type===pT});document.getElementById('paBody').innerHTML=!l.length?'<tr><td colspan="6" class="empty">None</td></tr>':l.map(function(p){return'<tr><td class="bold">'+p.name+'</td><td class="text-muted">'+(p.phone||'')+'</td><td class="text-muted">'+(p.address||'')+'</td><td class="r">'+(p.creditLimit?fmt(p.creditLimit):'--')+'</td><td class="r bold '+((p.balance||0)>0?'text-danger':'text-success')+'">'+fmt(Math.abs(p.balance||0))+((p.balance||0)>0?' Dr':p.balance<0?' Cr':'')+'</td><td class="r"><button class="btn btn-outline btn-sm" onclick="editPa(\''+p._key+'\')">Edit</button></td></tr>'}).join('')}
...
<div class="form-row"><div><label>Paid</label><input type="number" id="purPaid" value="0"></div><div><label>Payment Method</label><select id="purMethod"><option value="cash">Cash</option><option value="bank">Bank</option></select></div></div>
...
<script>
var purs=[],prods=[],suppliers=[];
async function loadPur(){var d=await Promise.all([loadList('purchase:'),loadList('product:'),loadList('party:')]);purs=d[0];prods=d[1];suppliers=d[2].filter(function(p){return p.type==='supplier'});
...
<div class="form-row"><div><label>Payment Method</label><select id="salMethod"><option value="cash">Cash</option><option value="bank">Bank</option></select></div><div></div></div>
...
<script>
var sals=[],salProds=[],customers=[],spList=[];
async function loadSal(){var d=await Promise.all([loadList('sale:'),loadList('product:'),loadList('party:'),loadList('salesperson:')]);sals=d[0];salProds=d[1];customers=d[2].filter(function(p){return p.type==='customer'});spList=d[3];
...
var data={invoiceNo:document.getElementById('salNo').value,date:document.getElementById('salDate').value,customerId:cid,customerName:cust?cust.name:'',salespersonId:spId,salespersonName:sp?sp.name:'',items:items.map(function(x){return{productId:x.pk,productName:x.pn,qty:x.qty,rate:x.rate,amount:x.amt}}),discount:dv,discountType:dt,extra:extra,vat:vv,vatType:vt,ait:av,aitType:at,total:total,paid:paid,method:document.getElementById('salMethod').value};
...
window.openTransfer=function(){document.getElementById('payType').value='transfer';document.getElementById('payTitle').textContent='Fund Transfer';document.getElementById('payNo').value=txnNo('TRF');document.getElementById('payDt').value=todayISO();document.getElementById('payAmt').value='';document.getElementById('payNote').value='';var accs=['Cash'].concat(banks.map(function(b){return b.name}));document.getElementById('payParty').innerHTML=accs.map(function(a){return'<option value="'+a+'">'+a+'</option>'}).join('');document.getElementById('billSelection').classList.add('hidden');
var el=document.getElementById('payModal').querySelector('.modal');el.querySelector('h3').textContent='Fund Transfer';
openModal('payModal')}
...
<div class="form-row"><div><label>Amount</label><input type="number" id="expAmt" placeholder="0"></div><div><label>Method</label><select id="expMeth"><option value="cash">Cash</option><option value="bank">Bank</option></select></div></div>
...
<script>
var exps=[],expHeads=[],expSubHeads=[];
async function loadExp(){var d=await Promise.all([loadList('expense:'),loadList('exphead:'),loadList('expsubhead:')]);exps=d[0];expHeads=d[1];expSubHeads=d[2];
...
window.saveExp=async function(){var ek=document.getElementById('expEK').value;var h=document.getElementById('expHead').value;if(!h)return alert('Select head');var amt=+document.getElementById('expAmt').value||0;if(amt<=0)return alert('Enter amount');var data={expenseNo:document.getElementById('expNo').value,date:document.getElementById('expDt').value,headName:h,subHeadName:document.getElementById('expSub').value,amount:amt,method:document.getElementById('expMeth').value,description:document.getElementById('expDesc').value};if(ek){await saveByKey(ek,data)}else{await saveItem('expense:',data)}closeModal('expModal');loadExp()}
window.editExp=function(k){var e=exps.find(function(x){return x._key===k});if(!e)return;document.getElementById('expEK').value=k;document.getElementById('expNo').value=e.expenseNo||'';document.getElementById('expDt').value=e.date;document.getElementById('expHead').value=e.headName||'';loadSubHeadOpts();setTimeout(function(){document.getElementById('expSub').value=e.subHeadName||''},100);document.getElementById('expAmt').value=e.amount;document.getElementById('expMeth').value=e.method||'cash';document.getElementById('expDesc').value=e.description||'';openModal('expModal')}
...
window.convertOrd=async function(k){var o=ords.find(function(x){return x._key===k});if(!o)return;if(!confirm('Convert to sales invoice?'))return;
var saleData={invoiceNo:txnNo('INV'),date:todayISO(),customerId:o.customerId||'',customerName:o.customerName,salespersonId:o.spId||'',salespersonName:o.spName||'',items:o.items||[],discount:0,discountType:'amount',extra:0,vat:0,vatType:'amount',ait:0,aitType:'amount',total:o.total,paid:0,method:'cash'};
await saveItem('sale:',saleData);o.status='converted';await saveByKey(k,cleanForSave(o));alert('Invoice created: '+saleData.invoiceNo);loadOrd()}
loadOrd();
</script>`}

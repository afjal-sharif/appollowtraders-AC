...
window.salFilterCust=function(){var q=(document.getElementById('salCustSearch').value||'').toLowerCase();var list=document.getElementById('salCustList');if(!q){list.innerHTML=customers.map(function(c){return'<div class="sr-item" onmousedown="salPickCust(\x27'+c._key+'\x27,\x27'+c.name.replace(/'/g,"")+'\x27)">'+c.name+'</div>'}).join('');list.classList.add('open');return}var fl=customers.filter(function(c){return c.name.toLowerCase().includes(q)||(c.phone||'').includes(q)||(c.address||'').toLowerCase().includes(q)});list.innerHTML=fl.length?fl.map(function(c){return'<div class="sr-item" onmousedown="salPickCust(\x27'+c._key+'\x27,\x27'+c.name.replace(/'/g,"")+'\x27)">'+c.name+(c.phone?' <span style=\"font-size:10px;color:var(--muted)\">'+c.phone+'</span>':'')+'</div>'}).join(''):'<div class="sr-item" style="color:var(--muted)">No match</div>';list.classList.add('open')};

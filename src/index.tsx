...
app.use('*', async (c, next) => {
  ...
  // Check for salesperson portal access (uses different cookie)
  if (path.startsWith('/sp-portal')) {
    const cookie = c.req.header("Cookie") || "";
    const spMatch = cookie.match(/sp_session=([^;]+)/);
    if (path === '/sp-portal/login' && c.req.method === 'GET') return htmlRes(spLoginPage(""));
    if (!spMatch && path !== '/sp-portal/login') return htmlRes(spLoginPage(""));
    await next(); return;
  }
  ...
  if (denied.includes(path)) {
    return htmlRes(layout('<div class="card"><div class="empty" style="padding:40px"><h3>Access Denied</h3><p class="text-muted">You don\'t have permission to access this page.</p><a href="/" class="btn btn-primary" style="margin-top:12px">Go to Dashboard</a></div></div>', 'dashboard', session));
  }
...
app.post('/api/set-license', async (c) => {
  const url = new URL(c.req.url);
...
app.post('/sp-portal/login', async (c) => {
  const form = await c.req.formData();
...
// ============================================================
function layout(content: string, active: string, session: any) {
  const role = session?.role || 'admin';
  ...
  const nav = [
    ...
    { group: 'Transactions', items: [
      { path:"/purchases", icon:"shopping_cart", label:"Purchases", id:"purchases" },
  ...
  let navHTML = '';
  for (const g of nav) {
    const visibleItems = g.items.filter(n => allowed.includes(n.id));
    if (visibleItems.length === 0) continue;
    navHTML += `<div class="nav-group">${g.group}</div>`;
    for (const n of visibleItems) {
      navHTML += `<a href="${n.path}" class="${active===n.id?'active':''}"><span class="nav-icon material-symbols-outlined">${n.icon}</span>${n.label}</a>`;
    }
  }
...
function dayDetailsPage(){return `
...
function salespersonPage(){return `

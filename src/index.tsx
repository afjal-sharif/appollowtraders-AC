...
// ============================================================
// BizManager — Apollow Traders — Cloudflare Worker + Hono
...
app.post('/login', async (c) => {
  ...
  if (form.get("pin") === PIN) {
    return new Response(null, { status: 302, headers: { "Set-Cookie": "auth=1; Path=/; HttpOnly; SameSite=Strict", Location: "/" } });
  }
  return htmlRes(loginPage("Wrong PIN"));
});

app.get('/logout', () => {
  return new Response(null, { status: 302, headers: { "Set-Cookie": "auth=; Path=/; HttpOnly; Max-Age=0", Location: "/login" } });
});

// License + Auth check middleware
app.use('*', async (c, next) => {
  const path = new URL(c.req.url).pathname;
  if (path === '/login' && c.req.method === 'GET') return htmlRes(loginPage(""));
  ...
  if (!loggedIn && path !== '/login') return htmlRes(loginPage(""));
...
app.post('/api/export-all', async (c) => {
  const prefixes = ['product:','party:','sale:','purchase:','payment:','expense:','exphead:','expsubhead:','bank:','cb:'];
  const all: any = {};
...
// Change PIN (stored in KV)
app.post('/api/change-pin', async (c) => {
  const { oldPin, newPin } = await c.req.json();
  const storedPin = await c.env.DATA_STORE?.get("APP_PIN") || PIN;
...
// PAGE ROUTES
// ============================================================
app.get('/', (c) => htmlRes(layout(dashboardPage(), "dashboard")));
app.get('/inventory', (c) => htmlRes(layout(inventoryPage(), "inventory")));
app.get('/parties', (c) => htmlRes(layout(partiesPage(), "parties")));
app.get('/purchases', (c) => htmlRes(layout(purchasesPage(), "purchases")));
app.get('/sales', (c) => htmlRes(layout(salesPage(), "sales")));
app.get('/payments', (c) => htmlRes(layout(paymentsPage(), "payments")));
app.get('/expenses', (c) => htmlRes(layout(expensesPage(), "expenses")));
app.get('/ledger', (c) => htmlRes(layout(ledgerPage(), "ledger")));
app.get('/profit-loss', (c) => htmlRes(layout(profitLossPage(), "profitloss")));
app.get('/day-details', (c) => htmlRes(layout(dayDetailsPage(), "daydetails")));
app.get('/admin', (c) => htmlRes(layout(adminPage(), "admin")));

app.get('/login', (c) => htmlRes(loginPage("")));
...
// LAYOUT
// ============================================================
function layout(content: string, active: string) {
  const nav = [
  ...
  const navHTML = nav.map(n =>
    `<a href="${n.path}" class="${active===n.id?'active':''}"><span class="nav-icon material-symbols-outlined" style="font-size:20px">${n.icon}</span>${n.label}</a>`
  ).join('');
...
<div class="app">
  <aside class="sidebar" id="sidebar">
    ...
    <nav>${navHTML}<a href="/logout" style="margin-top:24px;opacity:.6"><span class="nav-icon material-symbols-outlined" style="font-size:20px">logout</span>Logout</a></nav>
...
function expiredPage() {
  return `<!doctype html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>License Expired</title><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet">${getCSS()}</head><body>
...
<div class="card" style="margin-bottom:16px"><div class="form-row" style="grid-template-columns:1fr 1fr auto;align-items:end"><div><label>From</label><input type="date" id="plFrom" value="${from}" onchange="calcPL()"></div><div><label>To</label><input type="date" id="plTo" value="${to}" onchange="calcPL()"></div><div><button class="btn btn-primary" onclick="printPL()">Print</button></div></div></div>
...
<div class="card" style="max-width:420px;margin-bottom:16px">
  <div style="display:flex;align-items:end;gap:12px">
    ...
    <div style="flex:1"><label>Select Date</label><input type="date" id="dayDate" value="${today}" onchange="renderDay()"></div>
    <div class="date-nav">

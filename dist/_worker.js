...
openModal('payModal');
var existingBillKeys=p.billKeys||[];showBillsWithChecked(type,p.party||'',existingBillKeys);
document.getElementById('payParty').onchange=function(){showBills(type,this.value)}}
window.showBills=function(type,party){showBillsWithChecked(type,party,[])}
window.showBillsWithChecked=function(type,party,checkedKeys){var div=document.getElementById('billSelection');var list=document.getElementById('billList');if(!party){div.classList.add('hidden');return}
var bills=[];if(type==='receipt'){bills=paySales.filter(function(s){return s.customerName===party&&((s.total||0)-(s.paid||0)>0||checkedKeys.indexOf(s._key)>=0)}).map(function(s){return{no:s.invoiceNo,total:s.total,paid:s.paid,due:(s.total||0)-(s.paid||0),key:s._key}})}else{bills=payPurchases.filter(function(p){return p.supplierName===party&&((p.total||0)-(p.paid||0)>0||checkedKeys.indexOf(p._key)>=0)}).map(function(p){return{no:p.purchaseNo,total:p.total,paid:p.paid,due:(p.total||0)-(p.paid||0),key:p._key}})}
if(bills.length){div.classList.remove('hidden');list.innerHTML='<table class="tbl" style="font-size:11px"><thead><tr><th>Bill#</th><th class="r">Total</th><th class="r">Paid</th><th class="r">Due</th><th>Select</th></tr></thead><tbody>'+bills.map(function(b){var isChk=checkedKeys.indexOf(b.key)>=0;return'<tr><td>'+b.no+'</td><td class="r">'+fmt(b.total)+'</td><td class="r">'+fmt(b.paid)+'</td><td class="r text-danger">'+fmt(b.due)+'</td><td><input type="checkbox" data-key="'+b.key+'" data-due="'+b.due+'" onchange="calcBillAmt()"'+(isChk?' checked':'')+' ></td></tr>'}).join('')+'</tbody></table>'}else{div.classList.add('hidden')}}
...
if(ek){
  ...
  // Collect new bill selections from the edit form
  var editBills=[];document.querySelectorAll('#billList input[type=checkbox]:checked').forEach(function(c){editBills.push(c.dataset.key)});data.billKeys=editBills;
  ...
  // Apply new bill-wise payment effects on sale.paid / purchase.paid and update method
  if(editBills.length){var editRemaining=amt;for(var ei=0;ei<editBills.length&&editRemaining>0;ei++){var ebk=editBills[ei];if(type==='receipt'){var eSale=paySales.find(function(x){return x._key===ebk});if(eSale){var eDue=(eSale.total||0)-(eSale.paid||0);var eApply=Math.min(editRemaining,eDue);eSale.paid=(eSale.paid||0)+eApply;editRemaining-=eApply;if(eSale.paid>0&&eSale.method==='credit'){eSale.method=data.method||'cash'}await saveByKey(ebk,cleanForSave(eSale))}}else{var ePur=payPurchases.find(function(x){return x._key===ebk});if(ePur){var eDue2=(ePur.total||0)-(ePur.paid||0);var eApply2=Math.min(editRemaining,eDue2);ePur.paid=(ePur.paid||0)+eApply2;editRemaining-=eApply2;if(ePur.paid>0&&ePur.method==='credit'){ePur.method=data.method||'cash'}await saveByKey(ebk,cleanForSave(ePur))}}}}
...
if(bills.length){var remaining=amt;for(var i=0;i<bills.length&&remaining>0;i++){var bk=bills[i];if(type==='receipt'){var sale=paySales.find(function(x){return x._key===bk});if(sale){var due=(sale.total||0)-(sale.paid||0);var apply=Math.min(remaining,due);sale.paid=(sale.paid||0)+apply;remaining-=apply;if(sale.paid>0&&sale.method==='credit'){sale.method=data.method||'cash'}await saveByKey(bk,cleanForSave(sale))}}else{var pur=payPurchases.find(function(x){return x._key===bk});if(pur){var due2=(pur.total||0)-(pur.paid||0);var apply2=Math.min(remaining,due2);pur.paid=(pur.paid||0)+apply2;remaining-=apply2;if(pur.paid>0&&pur.method==='credit'){pur.method=data.method||'cash'}await saveByKey(bk,cleanForSave(pur))}}}}

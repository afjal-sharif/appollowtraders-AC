...
sales.forEach(function(s){var rcpts=rptPayments.filter(function(p){return p.type==='receipt'&&p.status==='done'&&p.party===s.customerName&&(p.billKeys||[]).indexOf(s._key)>=0});var rcptAmt=rcpts.reduce(function(a,r){return a+(r.amount||0)},0);var bal=(s.total||0)-(s.paid||0)-rcptAmt;tSale+=s.total||0;tInvP+=s.paid||0;tRcpP+=rcptAmt;invRows.push({cust:s.customerName,inv:s.invoiceNo,date:s.date,total:s.total||0,invPaid:s.paid||0,rcptPaid:rcptAmt,bal:Math.max(0,bal),key:s._key})});
